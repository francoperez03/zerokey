...
async function generateProof({ pan, expiryDate, cvv}) {
  try {
    ...
    const { witness } = await noir.execute({ pan, expiryDate, cvv, bankKey: "2"});
...
fastify.post('/proofs', async (request, reply) => {
  const { cvv, pan, expiryDate, email, name } = request.body;
  console.log({cvv, pan, expiryDate, email, name});
  const proofResult = await generateProof({ cvv, pan, expiryDate })
